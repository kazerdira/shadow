...
description: High-level architecture and design principles of shadow Robot.
---

import { Badge, Tabs, TabItem, Card, CardGrid } from '@astrojs/starlight/components';

shadow Robot is a modern Telegram group management bot built with Go and the gotgbot library. This document provides an overview of the architectural decisions, technology stack, and design principles that guide the codebase.
...
<Tabs>
  ...
  <TabItem label="Redis">
    ...
    All cache keys are prefixed with `shadow:` for namespace isolation. TTLs range from 20 seconds (anonymous admin verification) to 1 hour (language preferences).
  </TabItem>
...
<CardGrid>
  ...
  <Card title="Module System" icon="puzzle">
    Feature modules in `shadow/modules/` follow a consistent pattern: `moduleStruct`, handler methods, `LoadModule` registration. Help module loads last to collect all commands.
  </Card>
  ...
  <Card title="Permission System" icon="approve-check">
    Centralized permission validation in `shadow/utils/chat_status/`. Checks are cached to reduce Telegram API calls. Supports anonymous admin detection.
  </Card>
...
All database operations are organized by domain in `shadow/db/*_db.go` files. Each file provides Get/Add/Update/Delete functions for its domain, with surrogate key pattern (auto-increment `id` as PK, external IDs as unique constraints):
...
Middleware functionality is implemented through decorators in `shadow/utils/helpers/decorators.go`. Common cross-cutting concerns are handled uniformly:
...
4. Module loaded via `shadow/main.go` in specific order

### Cache Layer

Redis caching with stampede protection:

- **Cache Keys**: Prefixed with `shadow:` for namespace isolation
...
shadow uses a 4-layer error handling hierarchy:

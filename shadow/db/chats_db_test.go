...
func TestLoadChatStats(t *testing.T) {
	...
	t.Cleanup(func() {
		DB.Where("chat_id IN ?", []int64{chatIDActive, chatIDInactive}).Delete(&Chat{})
	})
	...
	if err := DB.Model(&Chat{}).Where("chat_id = ?", chatIDActive).Update("is_inactive", false).Error; err != nil {
	...
	if err := DB.Model(&Chat{}).Where("chat_id = ?", chatIDInactive).Update("is_inactive", true).Error; err != nil {
...
func TestLoadActivityStats(t *testing.T) {
	...
	t.Cleanup(func() {
		DB.Where("chat_id IN ?", []int64{chatIDDaily, chatIDWeekly, chatIDMonthly, chatIDOld}).Delete(&Chat{})
	})
	...
	if err := DB.Model(&Chat{}).Where("chat_id = ?", chatIDDaily).Updates(map[string]any{
	...
	if err := DB.Model(&Chat{}).Where("chat_id = ?", chatIDWeekly).Updates(map[string]any{
	...
	if err := DB.Model(&Chat{}).Where("chat_id = ?", chatIDMonthly).Updates(map[string]any{
	...
	if err := DB.Model(&Chat{}).Where("chat_id = ?", chatIDOld).Updates(map[string]any{

	...
	return &moduleBotClient{
		responses: map[string]json.RawMessage{
			...
			"getMe": json.RawMessage(
				`{"id":999,"is_bot":true,"first_name":"shadow","username":"shadowTestBot"}`,
			),
...
func (c *moduleBotClient) RequestWithContext(_ context.Context, _ string, method string, params map[string]any, _ *gotgbot.RequestOpts) (json.RawMessage, error) {
	...
	if method == "getChatMember" && fmt.Sprint(params["user_id"]) == "999" {
		return json.RawMessage(
			`{"status":"administrator","user":{"id":999,"is_bot":true,"first_name":"shadow"},"can_pin_messages":true,"can_delete_messages":true,"can_restrict_members":true,"can_promote_members":true,"can_change_info":true,"can_invite_users":true,"can_manage_chat":true}`,
		), nil
	}
...
func TestMain(m *testing.M) {
	...
	if db.DB == nil {
		dbFile, err := os.CreateTemp("", "shadow_modules_test_*.db")
		if err != nil {

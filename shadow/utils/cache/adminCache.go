...
func LoadAdminCache(b *gotgbot.Bot, chatId int64) AdminCache {
	...
	go func() {
		...
		for i := range maxRetries {
			if err := m.Set(Context, fmt.Sprintf("shadow:adminCache:%d", chatId), adminCache, store.WithExpiration(constants.AdminCacheTTL)); err != nil {
				log.WithFields(log.Fields{
					"chatId": chatId,
					"error":  err,
					"retry":  i + 1,
...
func GetAdminCacheList(chatId int64) (bool, AdminCache) {
	...
	gotAdminlist, err := m.Get(
		Context,
		fmt.Sprintf("shadow:adminCache:%d", chatId),
		new(AdminCache),
	)
	if err != nil {
		log.WithFields(log.Fields{
...
func GetAdminCacheUser(chatId, userId int64) (bool, gotgbot.MergedChatMember) {
	...
	adminList, err := m.Get(Context, fmt.Sprintf("shadow:adminCache:%d", chatId), new(AdminCache))
...
func InvalidateAdminCache(chatId int64) {
	...
	cacheKey := fmt.Sprintf("shadow:adminCache:%d", chatId)
